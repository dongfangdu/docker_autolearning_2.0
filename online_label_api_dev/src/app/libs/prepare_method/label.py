...
from app.libs.enums import UtteranceStatusEnum, PdataSrcTypeEnum, PrepareStatusEnum
...
def prepare_web_data(dp, p_uuid):
    ...
    with dp.app.app_context():
        ...
        ).order_by(desc(UtteranceAccess.time)).limit(100000)
        print q.count()
        rvs = q.all()
        total_cnt = len(rvs)
        ...
        while True:
            ...
            label_threshold_pre_wer = dp.app.config['LABEL_THRESHOLD_PRE_WER']
            threshold_length = dp.app.config['THRESHOLD_LENGTH']
            prepare_loop_counter += 1
            if request_id not in exclude_request_ids and request_id not in prepare_data_items.keys() and \
                    pre_wer > label_threshold_pre_wer:
