...
from app.models.v2.web.sysmgr import Role, User, RolePermissionMap, Permission
from app.validators.base import PageForm
from app.validators.forms_v2 import RoleForm, RoleEditForm, IDForm, RolePermissionMapForm
...
def role_add():
    ...
    with db_v2.auto_commit():
        ...
        db_v2.session.add(role)
    return ResultSuccess(msg=u'角色新增成功', data={'id': role.id, 'create_time': role.insert_time})
...
@auth.login_required
def role_choices():
    q = Role.query.filter_by().order_by(Role.id)

    # 权限约束
    cur_role = Role.query.filter(Role.id == User.rid, User.id == g.user.uid).first()
    if cur_role.rgroup == ChoicesExItemEnum.NEITHER.value:
        return ResultSuccess(msg=u'角色信息字典', data=[])
    if not cur_role.rgroup == 0:
        q = q.filter(Role.rgroup == cur_role.rgroup, Role.rlevel >= cur_role.rlevel)
    ...
    if choice_type == ChoicesExTypeEnum.TYPE_NEITHER.value:
        vms.append(dict(k=ChoicesExItemEnum.NEITHER.value, v={'rcode': u'-', 'rname': u'无'}))
    elif choice_type == ChoicesExTypeEnum.TYPE_ALL.value:
        vms.insert(0, dict(k=ChoicesExItemEnum.ALL.value, v={'rcode': u'-', 'rname': u'全部角色'}))
...
def role_code_by_uid(uid):
    _, role = db_v2.session.query(User, Role).filter(User.rid == Role.id).filter_by(id=uid).first_or_404()
    return role.rcode
