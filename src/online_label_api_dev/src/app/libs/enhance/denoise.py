...
from app.libs.enhance.data_loader import pre_emph
from app.libs.enhance.model import SEGAN, SEAE
...
flags.DEFINE_integer("denoise_epoch", 5,
                     "Epoch where noise in disc is removed (Def: 5).")
flags.DEFINE_integer("l1_remove_epoch", 150,
                     "Epoch where L1 in G is removed (Def: 150).")
...
flags.DEFINE_string("deconv_type", "deconv",
                    "Type of deconv method: deconv or nn_deconv (Def: deconv).")
...
flags.DEFINE_string("e2e_dataset", "data/segan.tfrecords",
                    "TFRecords (Def: data/segan.tfrecords.")
...
flags.DEFINE_string("weights", "SEGAN-97180", "Weights file")
FLAGS = flags.FLAGS
...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        test_wav_list = os.listdir(origin_data_path)
        for wav in test_wav_list:
            test_wav = os.path.join(origin_data_path, wav)
            fm, wav_data = wavfile.read(test_wav)
            wavname = test_wav.split('/')[-1].split('.')[0] + '.wav'
            if fm != 16000:
            ...
            wavfile.write(os.path.join(save_data_path, wavname), 16e3, c_wave)
            #print('Done cleaning {} and saved to {}'.format(test_wav,
                                 os.path.join(save_data_path, wavname)))


if __name__ == "__main__":
    origin_data_path = sys.argv[1]
    save_data_path = sys.argv[2]
    model_path = conf.config.ENH_MODEL_PATH
