...
def pitch_conversion(f0, mean_log_src, std_log_src, mean_log_target, std_log_target):
    ...
    explis = 0.0000001
    f0_converted = np.exp((np.log(f0 + explis) - mean_log_src) / std_log_src * std_log_target + mean_log_target)
...
def wav_write(file, output_dir, num, f0_converted, sp, ap, sampling_rate, frame_period):
    sp_like = np.zeros_like(sp)
    for f in xrange(sp_like.shape[1]):
        if np.mean(f0_converted) >= 200 and np.mean(f0_converted) <= 300:
            sp_like[:, f] = sp[:, int(f / 1.2)]
        elif np.mean(f0_converted) > 300:
            sp_like[:, f] = sp[:, int(f / 1.5)]
        else:
            sp_like = sp

    wav_transformed = world_speech_synthesis(f0=f0_converted, decoded_sp=sp_like, ap=ap, fs=sampling_rate, frame_period=frame_period)
    librosa.output.write_wav(os.path.join(output_dir, os.path.basename(file)[:-4] + "_{}.wav".format(num)), wav_transformed, sampling_rate)
