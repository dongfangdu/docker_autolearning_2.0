...
feats="$feats nnet-forward --no-softmax=true --prior-scale=1.0 --feature-transform=$feature_transform --class-frame-counts=$class_frame_counts --use-gpu=$use_gpu $nnet ark:- ark:- |"
...
if [ $stage -le 0 ]; then
  ...
  if [ $use_gpu == "yes" ];then
    ...
    #  nnet-align-compiled --use-gpu=$use_gpu $scale_opts --beam=$beam --retry-beam=$retry_beam "$mdl" ark:- \
    #    "$feats" "ark,scp,f:$dir/ali.JOB.ark,$dir/ali.JOB.scp" || exit 1;
    rm -f $dir/ali.*.scp $dir/ali.*.ark
    $cmd JOB=1:$nj $dir/log/align.JOB.log \
      compile-train-graphs $dir/tree $dir/final.mdl $lang/L.fst "$tra" ark:- \| \
      align-compiled-mapped $scale_opts --beam=$beam --retry-beam=$retry_beam $model ark:- \
        "$feats" "ark,scp,f:$dir/ali.JOB.ark,$dir/ali.JOB.scp" || exit 1;
  fi
fi

#mdl="gmm-boost-silence --boost=$boost_opt $optional_silence $dir/final.mdl - |" 
gmm-boost-silence --boost=$boost_opt $optional_silence $dir/final.mdl $dir/final.boost.mdl

if [ $stage -le 1 ]; then
  $cmd JOB=1:$nj $dir/log/ali2pdf.JOB.log \
    ali-to-pdf $dir/final.boost.mdl scp:$dir/ali.JOB.scp ark,scp:$dir/pdf.JOB.ark,$dir/pdf.JOB.scp
  #$cmd JOB=1:$nj $dir/log/ali2pdf.JOB.log \
  #  ali-to-pdf "$mdl" scp:$dir/ali.JOB.scp ark,scp:$dir/pdf.JOB.ark,$dir/pdf.JOB.scp
  $cmd JOB=1:$nj $dir/log/ali2post.JOB.log \
    ali-to-post scp:$dir/pdf.JOB.scp ark,scp:$dir/post.JOB.ark,$dir/post.JOB.scp
  $cmd JOB=1:$nj $dir/log/ali2phones.JOB.log \
    ali-to-phones $dir/final.mdl scp:$dir/ali.JOB.scp ark,t:$dir/phones.JOB.ark
fi

if [ $stage -le 2 ]; then
  #$cmd JOB=1:$nj $dir/log/phones2post.JOB.log \
  #	  python phone2post.py $dir/phones.JOB.ark $dir/post.tmp.JOB.ark
  for i in `seq 1 $nj`;do
    python $dir/phone2post.py $dir/phones.$i.ark $dir/post.tmp.$i.ark
    copy-post ark,t:$dir/post.tmp.$i.ark ark,scp:$dir/newpost.$i.ark,$dir/newpost.$i.scp
    #cp -rf $dir/post.$i.scp $dir/newpost.$i.scp
    #sed -i 's/\/post/\/newpost/g' $dir/newpost.$i.scp
    rm -f $dir/post.tmp.$i.ark
  done
fi 
